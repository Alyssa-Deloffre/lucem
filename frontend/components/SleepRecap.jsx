...
export default function SleepRecap({ eventInfos }) {
    ...
    const nightWakeToDisplay =
        event &&
        event.ref.nightwaking.map((nightWake, i) => {
            return (
                <View
                    key={i}
                    style={styles.nightWake}
                >
                    <View style={{ flexDirection: 'row' }}>
                        <Text style={styles.body}>Réveil nocturne à </Text>
                        <Text style={styles.bodyBold}>
                            {formatTime(new Date(nightWake.start))}
                        </Text>
                    </View>
                    <View style={{ flexDirection: 'row' }}>
                        <Text style={styles.body}>pendant </Text>
                        <Text style={styles.bodyBold}>
                            {formatDuration(new Date(nightWake.duration))}
                        </Text>
                    </View>
                </View>
            );
    ...
    const sleepQualityValue =
        event &&
        sleepQuality.find((item) => item.value === event.ref.sleepquality).text;
    const wakeQualityValue =
        event &&
        wakeQuality.find((item) => item.value === event.ref.wakingquality).text;
        ...
        <Card>
            <ScrollView>
                <View style={{ flexDirection: 'row' }}>
                    ...
                    {event && (
                        <Text style={styles.bodyBold}>
                            {formatTime(new Date(event.ref.start))}
                        </Text>
                    )}
                    <Text style={styles.body}> à </Text>
                    {event && (
                        <Text style={styles.bodyBold}>
                            {formatTime(new Date(event.ref.end))}
                        </Text>
                    )}
                </View>
