...
<ScrollView style={styles.scrollView}>
  <Text style={styles.titre}>Récap sommeil</Text>
    <LineChart
    data={{
      labels: dateSleepArr.length > 0 ? dateSleepArr?.map(date => date.getDate() + "/" + date.getMonth()) : ["test"],
      datasets: [
        {
          data: eventSleepArr.length > 0 ? eventSleepArr?.map(event => event.ref.sleepquality) : [0]
        }
      ]
    }}
